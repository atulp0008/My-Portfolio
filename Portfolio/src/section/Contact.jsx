...
export default function Contact() {
  ...
  const validateForm = () => {
    ...
    const newError = {};
    required.forEach(
      (f) => !formData[f].trim() && (newError[f] = "Fill this Field"),
    );
    setErrors(newError);
      ...
      <div className="relative z-10 w-full flex flex-col md:flex-row items-center gap-10">
        ...
        >
          <motion.img
            src={Astra}
            alt="contactPic"
            className="w-72  md:w-110 rounded-2xl shadow-lg object-cover"
            animate={{ y: [0, -10, 0] }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
          />
        </motion.div>
        <motion.div
          className="w-full p-5 md:w-1/2 bg-white/5 rounded-2xl shadow-lg border-white/10"
          initial={{ opacity: 0, x: 50 }}
          whileInView={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.6 }}
        >
          ...
          <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
            <div className="flex flex-col">
              ...
              />
              {errors.name && (
                <p className="text-red-500 text-sm ">Please fill the name</p>
              )}
            </div>
            <div className="flex flex-col">
              <label className="mb-1">
              ...
              />
              {errors.email && (
                <p className="text-red-500 text-sm ">Please fill the Email</p>
              )}
            </div>
            <div className="flex flex-col">
              <label className="mb-1">
              ...
              ></textarea>
              {errors.message && (
                <p className="text-red-500 text-sm ">Please fill the Message</p>
              )}
            </div>
            {status && (
              <p
